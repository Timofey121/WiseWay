...
## Постоянный архивный индекс

Режим включается отдельно, после резервной копии и проверки на своём корпусе:

```sh
docker compose --profile archive up -d indexer
docker compose logs --tail 100 indexer
docker compose run --rm cli status
```

`indexer` выполняет `index-archive --interval 60`: потоково обходит поисковые корни, записывает SQLite/FTS5 порциями и публикует полное поколение одной транзакцией. Обычный `worker` продолжает обработку поступлений. До первой успешной публикации доступен прежний индекс; при отказе архивного процесса он перестаёт обновляться. `doctor` проверяет возраст последнего полностью успешного архивного цикла. Первый большой импорт может превышать `WISEWAY_WORKER_STALE_SECONDS`; это реальная неготовность свежего индекса, а не причина автоматически перезапускать процесс.

Для запуска без Docker предусмотрен `infra/systemd/wiseway-indexer.service`; разовый проход — `uv run wiseway index-archive`. SIGTERM проверяется между порциями. Незавершённый обход не публикуется и при следующем запуске начинается заново; устойчивого продолжения большого обхода с последнего файла пока нет. Любое изменение требует построения нового поколения. Места должно хватать одновременно на старое и новое поколения, staging, WAL и временные результаты поиска.

Лимиты обычного Compose (1 GiB RAM, 2 CPU, 64 MiB `/tmp` на процесс) относятся к демостенду. SQLite создаёт временные таблицы поиска в файловом TEMP; в этом Compose `/tmp` — tmpfs, поэтому расход входит в память контейнера. Для большого корпуса лимиты и место под TEMP задаются отдельным Compose override после замера, а не увеличением числа API workers. Параллельные запросы умножают потребность во временном месте. Ограничения и измерения: [performance.md](performance.md).
...
Команда останавливает API/worker и работающий архивный indexer, создаёт согласованную копию `data/` и `sandbox/` в новом подкаталоге backups, затем возвращает только ранее работавшие сервисы. **Во время копирования API недоступен.** Snapshot проверяет SQLite, завершённую инициализацию, файлы и пустые каталоги; manifest содержит SHA-256. Ссылки, специальные файлы, существующее назначение и незавершённые durable-операции отклоняются. При таком отказе разберите `status`, завершите обработку и повторите копирование под новым именем.

Этот полный snapshot предназначен для демопесочницы: обход ограничен 100 000 объектами и manifest размером 16 MiB. Он не является средством копирования миллиона файлов или 200 ТБ. Для промышленного архива нужны согласованные снимки хранилища и отдельные резервные копии служебной БД/поискового индекса; процедура и RTO проверяются на фактическом хранилище.

Прямой `snapshot.py` исключает одновременный запуск archive indexer файловой блокировкой, но не останавливает API/worker: вызывайте его только через orchestration либо после самостоятельной остановки всех писателей. После аварийного прерывания host-команды убедитесь, что временный CLI-контейнер больше не копирует файлы, прежде чем запускать сервисы.
...
# Если включён профиль archive, также остановите indexer.
docker compose --profile archive stop indexer
...
# Для установки с постоянным архивным индексом:
docker compose --profile archive up -d --force-recreate indexer
python3 infra/scripts/compose_ops.py check
```

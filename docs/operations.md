...
Ниже описан запуск без контейнеров. Docker Compose, pipeline, согласованные snapshot/restore и отдельные host timers описаны в [deployment.md](deployment.md). Выберите один вариант запуска API/worker для рабочей базы.

Этот документ описывает текущую синтетическую поставку для одного Linux-сервера и не более 50 пользователей. Запускайте API и worker отдельными постоянно перезапускаемыми службами от выделенного непривилегированного пользователя. Он один владеет каталогами состояния, резервных копий и sandbox; посторонние процессы и пользователи не должны менять файлы в этих каталогах.
...
Дальнейшие команды выполняйте от этого же пользователя с тем же env-файлом. Используйте установленный `/opt/wiseway/.venv/bin/wiseway`: запуск не требует изменения окружения в каталоге кода. Журналы служб доступны через `journalctl -u wiseway-api -u wiseway-worker`; проверяйте также ошибки `wiseway-backup.service`. Срабатывание timer само по себе не подтверждает успешную копию.

`WISEWAY_TRUST_PROXY_HEADERS=true` допустим только при loopback-привязке API и reverse proxy на том же сервере. Proxy завершается TLS, передаёт только ожидаемые forwarded headers (включая `X-Forwarded-For`) и не предоставляет API напрямую извне. Без него приложение не принимает proxy headers. Worker запускается отдельно: `/opt/wiseway/.venv/bin/wiseway worker`.

Перед открытием доступа проверьте `/opt/wiseway/.venv/bin/wiseway doctor`; после запуска worker он должен показывать `ready: true`. При неготовности команда возвращает exit code 1. Порог устаревания heartbeat задаёт `WISEWAY_WORKER_STALE_SECONDS` (минимум 60, по умолчанию 900). `/opt/wiseway/.venv/bin/wiseway status` показывает failed indexes и незавершённые recovery; `/opt/wiseway/.venv/bin/wiseway recover ATTEMPT_ID` применяют только после проверки фактического расположения файла. Worker обрабатывает SIGTERM: заканчивает текущий этап перед остановкой; при принудительном kill остаётся механизм durable recovery.
...
Каждый час создавайте новую резервную копию SQLite вне state и sandbox, например `/opt/wiseway/.venv/bin/wiseway backup /var/backups/wiseway/wiseway-YYYYMMDDHH.sqlite3`. Команда создаёт новый файл и проверяет его целостность. Она не архивирует sandbox: для восстановления бизнес-состояния резервируйте sandbox согласованно с базой и проверяйте процедуру на отдельном хосте.

Для каждой резервной копии запускайте `/opt/wiseway/.venv/bin/wiseway restore-check BACKUP DESTINATION` с новым пустым destination. Команда проверяет копию базы; она не переключает рабочий сервер и не восстанавливает файлы. Автоматического retention нет: оператор определяет срок, шифрование, доступ и удаление копий.
...
Обычное копирование sandbox меняет inode файлов. Даже согласованная копия может поэтому потребовать ручного разбора незавершённых намерений. Новый `infra/scripts/snapshot.py` запрещает создание/восстановление таких копий при незавершённых операциях, сохраняет пустые каталоги и проверяет хэши. Для systemd сначала остановите обе службы, выполните `python3 infra/scripts/snapshot.py create --data-dir /var/lib/wiseway/state --sandbox-dir /srv/wiseway/sandbox --destination /var/backups/wiseway/NEW_NAME` от пользователя сервиса, затем запустите службы. Полный процесс проверки и выбора новых каталогов описан в [deployment.md](deployment.md); `compose_ops.py` предназначен только для контейнерного варианта.

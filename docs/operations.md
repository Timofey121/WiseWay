...
Перед открытием доступа проверьте `/opt/wiseway/.venv/bin/wiseway doctor`; после запуска worker он должен показывать `ready: true`. Порог устаревания heartbeat задаёт `WISEWAY_WORKER_STALE_SECONDS` (минимум 60, по умолчанию 900). `/opt/wiseway/.venv/bin/wiseway status` показывает failed indexes и незавершённые recovery; `/opt/wiseway/.venv/bin/wiseway recover ATTEMPT_ID` применяют только после проверки фактического расположения файла.
...
Обычное копирование sandbox меняет inode файлов. Даже согласованная копия может поэтому потребовать ручного разбора незавершённых намерений: сохранённая идентичность уже не совпадает, и worker оставляет `RECOVERY_REQUIRED`, сохраняя файлы.

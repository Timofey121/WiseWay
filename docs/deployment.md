...
Настройте обязательные checks для `main` в правилах репозитория. Для повторного выпуска увеличьте версию: workflow запрещает перезаписывать опубликованные refs. GitHub-hosted проверки и публикация выполнятся после отправки изменений и запуска workflow; наличие YAML не означает успешный remote run.

...
Обычный демообход ограничен 10 000 записями каталога, глубиной 64 и суммарными 8 MiB имён путей; между записями проверяется бюджет 30 секунд. Для большого поискового корпуса есть отдельный `uv run wiseway index-archive --interval 60` с постоянным SQLite/FTS5-индексом; ограничения и порядок включения описаны в [инструкции](docs/deployment.md#постоянный-архивный-индекс). При отказе предыдущий полный индекс сохраняется. При утрате базы повторный `init-demo` отклоняется; существующие файлы и пароли не пересоздаются.
...
[Замеры и проектирование под 200–400 млн файлов](docs/performance.md): воспроизводимые стенды, реализованные оптимизации и работа, необходимая для промышленного архива.

Для больших архивов добавлены [OpenSearch и потоковый импорт дельт](docs/archive-search.md), контрольные точки и отдельная CI-проверка реального движка. Промышленный источник изменений и проверка 200–400 млн записей остаются отдельными этапами.
